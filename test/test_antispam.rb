...
class TestAntiSpam < Test::Unit::TestCase

  include GreenEggsAndSpam
  
  should "get key index" do
    assert GreenEggsAndSpam::AntiSpam.key_index.is_a?(Hash)
  end
  
  should "set key index" do
    index = { "1" => "green", "2" => "pink" }
    assert GreenEggsAndSpam.options[:key_index] = index
    assert_equal "green", AntiSpam.key_index["1"]
    assert_equal "pink", AntiSpam.key_index["2"]
  end
  ...
  should "set key index to things other than numbers" do
    ...
    assert_equal "green", AntiSpam.key_index["lime"]
    assert_equal "pink", AntiSpam.key_index["grapefruit"]
  end

  should "generate a random key" do
    key = AntiSpam.random_key
    assert AntiSpam.key_index.include?(key)
  end
